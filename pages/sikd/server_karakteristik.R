...
data <- data %>%
  mutate(jenis_kelamin = ifelse(jenis_kelamin == 1, "Perempuan", "Laki-Laki"),
         Pendidikan = recode(Pendidikan,
                             ...
                             `6` = "S2",
                             `7` = "S3"),
         skala_usaha = recode(skala_usaha,
...
output$data_table <- renderDT({
  data_display <- data %>%
    rename(`Jenis Kelamin` = jenis_kelamin, `Skala Usaha` = skala_usaha) 
  
  datatable(data_display, options = list(
    headerCallback = JS(
      "function(thead, data, start, end, display){",
      "  if (!$('#toggle-age-chart').length) {",
      "  $(thead).closest('thead').prepend('<tr style=\"position: relative;top: 10px;\"> <th style=\"border: none; padding: 0px 10px 0px 14px;\"></th> <th style=\"border: none; padding: 0px 10px 0px 14px;\"></th> <th style=\"border: none; padding: 0px 10px 0px 14px;\"></th> <th style=\"border: none; padding: 0px 10px 0px 14px;\"><input style=\"transform: scale(1.2);\" type=\"checkbox\" id=\"jenis-kelamin\"></th> <th style=\"border: none; padding: 0px 10px 0px 14px;\"><input style=\"transform: scale(1.2);\" type=\"checkbox\" id=\"toggle-age-chart\"></th> <th style=\"border: none; padding: 0px 10px 0px 14px;\"><input style=\"transform: scale(1.2);\" type=\"checkbox\" id=\"pendidikan\"></th> <th style=\"border: none; padding: 0px 10px 0px 14px;\"><input style=\"transform: scale(1.2);\" type=\"checkbox\" id=\"pekerjaan-utama\"></th> <th style=\"border: none; padding: 0px 10px 0px 14px;\"><input style=\"transform: scale(1.2);\" type=\"checkbox\" id=\"pekerjaan-sampingan\"></th> <th style=\"border: none; padding: 0px 10px 0px 14px;\"><input style=\"transform: scale(1.2);\" type=\"checkbox\" id=\"memulai-usaha\"></th> <th style=\"border: none; padding: 0px 10px 0px 14px;\"><input style=\"transform: scale(1.2);\" type=\"checkbox\" id=\"jenis-usaha\"></th> <th style=\"border: none; padding: 0px 10px 0px 14px;\"><input style=\"transform: scale(1.2);\" type=\"checkbox\" id=\"skala-usaha\"></th></tr>');",
...
output$pie_chart_skala_usaha <- renderPlot({
  gender_count <- data %>%
    count(`skala_usaha`) %>%
    mutate(percentage = n / sum(n) * 100,
           label = paste0(skala_usaha, "\n", round(percentage, 1), "%"))
  
  ggplot(gender_count, aes(x = "", y = n, fill = skala_usaha)) +
    geom_bar(stat = "identity", width = 1) +
    coord_polar("y") +
    theme_void() +
    geom_text(aes(label = label), position = position_stack(vjust = 0.5)) +
